...
This tutorial is a brief introduction to causal inference with a focus on how to choose "control variables" in a Bayesian regression analysis, and a very brief introduction to using causal discovery algorithms to identify causal structure in data. It presumes knowledge of fitting Bayesian regression models with `brms` [@brms], but the methods discussed here also apply to linear regression.

For more detailed coverage of causal inference, see @rohrer2018thinking or @hernan2024causal. @malinsky2018causal provides a good introduction to causal discovery methods.
...
In this tutorial, we'll be looking at the `english` dataset from `languageR`. To keep things simple, we'll only use a subset of the variables included in this data:
...
We only care about the effect of `WrittenFrequency`, so let's exclude everything else from our regression. We'll use default `brms` priors here:
...
```{r}
#| code-fold: true
eng_m2_ce <- conditional_effects(eng_m2, effects = "WrittenFrequency_c")$WrittenFrequency_c

eng_m2_ce %>% ggplot(aes(x = WrittenFrequency_c, y = estimate__)) +
...
All of these variables could reasonable predict `RTlexdec`, so we could include them in a regression:

```{r}
eng_m3 <-
  brm(
    data = eng,
    RTlexdec ~ 1 + WrittenFrequency_c + Familiarity_c + FrequencyInitialDiphoneWord_c + AgeSubject_c,
    family = gaussian,
    file = "models/eng_m3.brm"
  )

summary(eng_m3)
```

In this model, we see a `WrittenFrequency` effect of -0.7 (95\% CredI: [-0.08, -0.06]). Like the previous model `eng_m2`, this is still negative, but much smaller.

#### 3. Incude the predictors that perform best under some model comparison procedure.

We could also fit multiple models, compare them, and choose the "best" model in our comparison. Using LOO, we would choose `eng_m3`, the model with all variables included.

```{r}
eng_m2 <- add_criterion(eng_m2, c("loo"))
eng_m3 <- add_criterion(eng_m3, c("loo"))

loo_compare(eng_m2, eng_m3, criterion = "loo")
```
...
This means we should use our first model, `eng_m2`, to estimate the causal effect of `WrittenFrequency` on `RTlexdec`, although it performed worse in our model comparison. The best *causal* model is not always the same as the best *predictive* model.
...
This gives us two possible adjustment sets. We can control for either `WrittenFrequency` or `Familiarity` to estimate the causal effect of `DiphoneFreq` on `RTlexdec`. We can fit both models with `brms`:

```{r}
eng_m4 <-
  brm(
    data = eng,
    RTlexdec ~ 1 + WrittenFrequency_c + FrequencyInitialDiphoneWord_c,
    family = gaussian,
    file = "models/eng_m4.brm"
  )
...
eng_m5 <-
  brm(
    data = eng,
    RTlexdec ~ 1 + Familiarity_c + FrequencyInitialDiphoneWord_c,
    family = gaussian,
    file = "models/eng_m5.brm"
  )
...
eng_m6 <-
  brm(
    data = eng,
    Familiarity_c ~ 1 + AgeSubject_c,
    family = gaussian,
    file = "models/eng_m6.brm"
  )
...
eng_m7 <-
  brm(
    data = eng,
    RTlexdec ~ 1 + WrittenFrequency_c + Familiarity_c,
    family = gaussian,
    file = "models/eng_m7.brm"
  )
